...
def gen_wf_from_folder(wf_filelist,key,lenData,channel_ID,eventID_string):
    """
    Note
    ----------
   ** MAKE NEW FOR EACH DATASET:: Add settings for your project key below
    ...
    for i, path in enumerate(wf_filelist):
        ...
        ## STANDARD:: event ID from filename
        # evID = path.split('/')[-1]
        evID = path.split('/')[-1].split('.')[-1]
        ...
        # Later maybe we can specify evID with function:
        evID = getEventID(path,key,eventID_string)
